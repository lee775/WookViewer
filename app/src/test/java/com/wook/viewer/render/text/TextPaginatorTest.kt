...
class TextPaginatorTest {
    ...
    @Test fun `짧은 단일 단락은 한 페이지`() {
        val pages = TextPaginator.paginate("안녕하세요. 짧은 문서입니다.")
        assertEquals(1, pages.size)
        assertTrue(pages[0].text.contains("안녕하세요"))
    }

    @Test fun `한 페이지 한도 초과 시 분리`() {
        val para = "한글텍스트 ".repeat(200)
    ...
    @Test fun `한 단락이 한도보다 길면 강제 분할`() {
        val huge = "X".repeat(TextPaginator.APPROX_CHARS_PER_PAGE * 3)
        val pages = TextPaginator.paginate(huge)
        assertTrue("강제 분할 페이지 ≥ 3: ${pages.size}", pages.size >= 3)
        assertEquals(TextPaginator.APPROX_CHARS_PER_PAGE, pages[0].text.length)
    }

    @Test fun `여러 단락이 한 페이지 안에`() {
